...
//aca le aplico el metodo router, con esto lo voy a poder enrutar al ruteo principal
...
//MI BASE DE DATOS la puedo tener en otro modulo
...
//desde mi front le envian una peticion a /product
...
server.post('/:name', (req, res) => {//NOS TRAEMOS POR PARAMETROS EL NOMBRE
    ...
    //el query lo declara el cliente
    let {price} = req.query// POR QUERY TRAEMOS EL PRECIO
    let {seller} = req.body// POR BODY TRAEMOS EL VENDEDOR

    // si me falta algunos de esos datos  
//aca tengo que preguntar si vino por querys por que no se                                          esto es lo que le llega al front
